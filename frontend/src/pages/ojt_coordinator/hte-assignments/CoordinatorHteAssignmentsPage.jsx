...
import { Search, X, ArrowUp, ArrowDown, ChevronsUpDown, UserCheck, Users, Building2 } from "lucide-react";
  ...
  return (
    <CoordinatorLayout>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-green-950 sm:text-3xl">Assigned Interns</h1>
          <p className="mt-1 text-sm text-gray-500">Assign your approved interns to host training establishments.</p>
        </div>
      </div>
        ...
        {!loading && rows.length > 0 && (
          <>
            <div className="space-y-2.5 p-3 sm:p-4 md:hidden">
              {rows.map((hte) => (
                <div key={hte.uuid} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm ring-1 ring-gray-100">
                  ...
                  <div className="mt-2.5 text-xs text-gray-500">{hte.program || "—"}</div>
                  <div className="mt-3 flex gap-2 border-t border-gray-50 pt-3">
                    ...
                    >
                      <UserCheck size={15} /> Assign interns
                    </Button>
                  </div>
                </div>
              ))}
