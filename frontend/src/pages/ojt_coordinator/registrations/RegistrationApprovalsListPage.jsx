...
import { registrationStatusOptions } from "@/pages/ojt_coordinator/registrations/constants.js";
...
export default function RegistrationApprovalsListPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load registrations", { description: firstErrorMessage(err) });
    } finally {
      setLoading(false);
    }
  }, [page, status, debouncedSearch, order, academicYear]);
  ...
  const hasFilters = Boolean(search) || status !== "pending" || academicYear !== (terms.find((t) => t.status === "active") ? String(terms.find((t) => t.status === "active").id) : "all");

  return (
    <CoordinatorLayout>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-green-950 sm:text-3xl">Registration Approvals</h1>
          <p className="mt-1 text-sm text-gray-500">Review intern registration requests for your institute.</p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm ring-1 ring-gray-100">
        ...
        {!loading && rows.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            ...
            <p className="text-sm font-semibold text-gray-700">No registrations found</p>
                ...
                <TableBody>
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <span className="text-sm text-gray-600">{row.program || "—"}</span>
                      </TableCell>
                      <TableCell>
                        <StatusChip status={row.status} />
                      </TableCell>
