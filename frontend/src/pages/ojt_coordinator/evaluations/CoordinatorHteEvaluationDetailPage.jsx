...
export default function CoordinatorHteEvaluationDetailPage() {
  const { uuid, hteUuid, internUuid } = useParams();
  const effectiveUuid = internUuid || uuid;
  const effectiveHteUuid = hteUuid || null;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const url = effectiveHteUuid
        ? `/coordinator/hte-evaluations/${effectiveHteUuid}/${effectiveUuid}`
        : `/coordinator/hte-evaluations/${effectiveUuid}`;
  ...
  }, [effectiveUuid, effectiveHteUuid]);
  ...
  const handlePrint = () => {
    ...
    setIsPrinting(true);
    const printUrl = effectiveHteUuid
      ? `/coordinator/hte-evaluations/${effectiveHteUuid}/${effectiveUuid}/print`
      : `/coordinator/hte-evaluations/${effectiveUuid}/print`;
    const ensureIframe = () => {
  ...
  return (
    <CoordinatorLayout>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          <Link to={effectiveHteUuid ? `/coordinator/hte-evaluations/${effectiveHteUuid}` : "/coordinator/hte-evaluations"}>
            <ArrowLeft size={16} /> {effectiveHteUuid ? "Back to interns" : "Back to HTE evaluations"}
          </Link>
        </Button>
        {data && !loading && !error && (
          <Button
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm ring-1 ring-gray-100">
          ...
          <Button asChild variant="outline" className="mt-4 h-10 rounded-xl text-green-700">
            <Link to="/coordinator/hte-evaluations">Back to intern evaluations</Link>
          </Button>
        </div>
      ) : data ? (
        <>
          ...
          <div className="flex items-start gap-2.5 rounded-2xl bg-blue-50 p-3.5 ring-1 ring-blue-200">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-sm leading-relaxed text-blue-800">
                This evaluation was submitted by the intern for the HTE. You are viewing it in read-only mode.
              </p>
            </div>
          </div>
          ...
          {totalCount === 0 && (
            <div className="flex flex-col items-center gap-2 rounded-2xl border border-gray-100 bg-white p-10 text-center shadow-sm ring-1 ring-gray-100">
              ...
              <p className="max-w-sm text-xs text-gray-400">No active HTE evaluation criteria for this institute.</p>
            </div>
          )}
