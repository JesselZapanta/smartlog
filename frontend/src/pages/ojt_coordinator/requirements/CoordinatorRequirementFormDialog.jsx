...
import { AlignLeft, ClipboardList, Loader2, Save, FileCheck2 } from "lucide-react";
import api from "@/lib/api";
import { firstErrorMessage } from "@/lib/errors";
import { statusOptions, toActiveValue, typeOptions } from "@/pages/admin/requirements/constants.js";
...
const formFieldNames = ["name", "description", "type", "is_active"];
...
export default function CoordinatorRequirementFormDialog({ open, onOpenChange, requirement, instituteName, onSaved }) {
      ...
      form.reset(
        requirement
          ? {
              ...
              type: requirement.type || "pre_deployment",
              status: toActiveValue(requirement.is_active),
            }
          : emptyValues
      );
    }
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/coordinator/requirements/${requirement.id}`, payload);
        toast.success("Requirement updated", { description: `${values.name} was updated.` });
      } else {
        await api.post("/coordinator/requirements", payload);
        toast.success("Requirement created", { description: `${values.name} was added.` });
      }
      onSaved();
    } catch (err) {
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <DialogDescription className="text-sm">
                {instituteName ? `For ${instituteName}` : "OJT requirement"} —{" "}
                {isEdit ? "update the details and status." : "create a new requirement."}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} noValidate className="space-y-6">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Requirement details</p>
              </div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <ClipboardList className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                          <Input
            ...
            <DialogFooter className="flex-col-reverse gap-2 border-t border-gray-100 pt-4 sm:flex-row sm:justify-end">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    <Save size={16} /> {isEdit ? "Save changes" : "Create requirement"}
                  </>
                )}
              </Button>
            </DialogFooter>
