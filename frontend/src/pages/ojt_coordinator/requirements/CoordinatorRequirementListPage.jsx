...
export default function CoordinatorRequirementListPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        if (!cancelled) setInstituteName(res.data.data.institute?.name || "");
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const loadRequirements = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/coordinator/requirements?${params.toString()}`);
  ...
  }, [page, type, status, debouncedSearch, order]);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`/coordinator/requirements/${deleteTarget.id}`);
  ...
  return (
    <CoordinatorLayout>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            OJT requirements for {instituteName || "your institute"}.
          </p>
        </div>
      ...
      <CoordinatorRequirementFormDialog
        open={dialogOpen}
