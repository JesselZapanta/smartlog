...
export default function InstructorInternMonitoringCalendarPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [internRes, recordsRes] = await Promise.all([
        ...
        api.get(`/instructor/interns/${uuid}/monitoring?month=${month}`),
      ]);
  ...
  const statusByDate = useMemo(
    () => new Map(journals.map((entry) => [entry.date, entry.status])),
    [journals]
  );
  ...
  const stats = useMemo(() => {
    const approved = journals.filter((entry) => entry.status === "checked").length;
    ...
    return { total: journals.length, approved, verified, rejected };
  }, [journals]);
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm ring-1 ring-gray-100">
          ...
          <Button asChild variant="outline" className="mt-4 h-10 rounded-xl text-green-700">
            <Link to="/instructor/monitoring">Back to intern monitoring</Link>
          </Button>
        </div>
      ) : intern ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <StatCard icon={CheckCircle2} label="Checked" value={stats.approved} tone="indigo" />
            <StatCard icon={Clock3} label="Verified" value={stats.verified} tone="emerald" />
            <StatCard icon={Flag} label="Rejected" value={stats.rejected} tone="red" />
          </div>
          ...
          <section className="overflow-hidden rounded-2xl border border-green-100 bg-white shadow-sm ring-1 ring-green-100">
            ...
            <div className="p-3 sm:p-5">
              <Calendar
                ...
                onSelect={(date) => {
                  if (date) navigate(`/instructor/monitoring/${uuid}/${dateKey(date)}`);
                }}
              ...
              <div className="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[11px] font-semibold text-gray-400">
                <span className="inline-flex items-center gap-1">
                  <Check size={12} strokeWidth={3.5} className="text-green-600" /> Approved
                </span>
                <span className="inline-flex items-center gap-1">
