...
import { ArrowLeft, ArrowRight, BookOpenText, ChevronLeft, ChevronRight, Clock3, Flag, Loader2, NotebookPen, ShieldCheck, X } from "lucide-react";
...
const JOURNAL_STATUS = {
  ...
  checked: { badge: "bg-green-50 text-green-700 ring-green-200", label: "Checked" },
  flagged: { badge: "bg-red-50 text-red-700 ring-red-200", label: "Flagged by HTE" },
  rejected: { badge: "bg-red-50 text-red-700 ring-red-200", label: "Rejected" },
};
...
function recordStatusText(status) {
  ...
  if (status === "verified") return "Verified by HTE";
  if (status === "flagged") return "Flagged by HTE";
  return "Pending";
}
...
export default function InstructorInternMonitoringDayPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [internRes, journalsRes] = await Promise.all([
        api.get(`/instructor/interns/${uuid}`),
        api.get(`/instructor/interns/${uuid}/monitoring?date=${dateParam}`),
      ]);
      setIntern(internRes.data.data);
      setDtr(journalsRes.data.dtr || null);
      setJournal(journalsRes.data.data || null);
    } catch (err) {
  ...
  function openVerifyDialog() {
    setVerifyDtr(Boolean(dtr && dtr.status !== "checked"));
    setVerifyJournal(Boolean(journal && journal.status !== "checked"));
    setVerifyOpen(true);
  }
  ...
  async function confirmVerify() {
    if (reviewing) return;
    const targets = [];
    if (verifyDtr && dtr && dtr.status !== "checked") targets.push("dtr");
    if (verifyJournal && journal && journal.status !== "checked") targets.push("journal");
    if (targets.length === 0) return;
    setReviewing("verify");
    try {
      for (const type of targets) {
        await api.post(`/instructor/interns/${uuid}/monitoring/approve`, { type, date: dateParam });
      }
      toast.success("Journal(s) checked");
      setVerifyOpen(false);
      await load();
    } catch (err) {
  ...
  function openFlagDialog() {
    setFlagDtr(Boolean(dtr && dtr.status !== "rejected"));
    setFlagJournal(Boolean(journal && journal.status !== "rejected"));
    setFlagOpen(true);
  }
  ...
  async function rejectRecords() {
    if (reviewing) return;
    const targets = [];
    if (flagDtr && dtr && dtr.status !== "rejected") targets.push("dtr");
    if (flagJournal && journal && journal.status !== "rejected") targets.push("journal");
    if (targets.length === 0 || !remarks.trim()) return;
    setReviewing("flag");
    try {
      for (const type of targets) {
        await api.post(`/instructor/interns/${uuid}/monitoring/reject`, {
          type,
          date: dateParam,
          remarks: remarks.trim(),
        });
      }
      toast.success("Journal(s) rejected");
      setRemarks("");
      setFlagOpen(false);
      await load();
    } catch (err) {
  ...
  return (
    <InstructorLayout>
      <div className="flex items-center gap-2">
        ...
        >
          <Link to={`/instructor/monitoring/${uuid}`}>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm ring-1 ring-gray-100">
          ...
          <Button asChild variant="outline" className="mt-4 h-10 rounded-xl text-green-700">
            <Link to={`/instructor/monitoring/${uuid}`}>Back to calendar</Link>
          </Button>
        </div>
      ) : intern ? (
        <>
          ...
          <section className="space-y-5">
            ...
            {(journal?.status === "flagged" || journal?.status === "rejected") && journal.remarks && (
              <div className="rounded-2xl border border-red-100 bg-red-50/60 p-4 shadow-sm ring-1 ring-red-100 sm:p-5">
                <div className="flex items-center gap-2">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-100 text-red-600">
                    <Flag size={18} />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-red-700">Remarks</p>
                    <p className="font-heading text-base font-bold text-red-800">{formatDate(dateParam)}</p>
                  </div>
                </div>
                <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-red-900">{journal.remarks}</p>
              </div>
            )}
          ...
          <Dialog open={verifyOpen} onOpenChange={setVerifyOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                    ...
                    <DialogDescription>
                      Select the journal(s) to mark as checked for {formatDate(dateParam)}.
                    </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex flex-col gap-2.5">
                <RecordCheckRow
                  title="Photo DTR"
                  checked={verifyDtr}
                  disabled={!dtr || dtr.status === "checked"}
                ...
                <RecordCheckRow
                  ...
                  disabled={!journal || journal.status === "checked"}
          ...
          <Dialog open={flagOpen} onOpenChange={setFlagOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                    <DialogTitle>Reject journals</DialogTitle>
                    <DialogDescription>
                      Select the journal(s) to reject for {formatDate(dateParam)} and add your remarks.
                    </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex flex-col gap-2.5">
                <RecordCheckRow
                  title="Photo DTR"
                  checked={flagDtr}
                      disabled={!dtr || dtr.status === "rejected"}
                  statusText={dtr ? recordStatusText(dtr.status) : undefined}
                ...
                <RecordCheckRow
                  title="Journal entry"
                  checked={flagJournal}
                      disabled={!journal || journal.status === "rejected"}
                  statusText={journal ? recordStatusText(journal.status) : undefined}
                  onCheckedChange={setFlagJournal}
                />
              </div>

                  <label htmlFor="reject-remarks" className="text-xs font-bold uppercase tracking-wider text-gray-400">
                    Remarks
                  </label>
                  <Textarea
                    id="reject-remarks"
