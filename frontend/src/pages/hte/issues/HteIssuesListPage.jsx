...
import HteIssueFormDialog from "@/pages/hte/issues/HteIssueFormDialog.jsx";
...
function StatusBadge({ status }) {
  ...
  return (
    <Badge className="rounded-full bg-amber-50 font-semibold text-amber-700 ring-1 ring-amber-200">Pending</Badge>
  );
}

export default function HteIssuesListPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/hte/issues?${params.toString()}`);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`/hte/issues/${deleteTarget.id}`);
  ...
  return (
    <HteLayout>
            <PageHeader
        title="Issues"
        subtitle="Track and manage issues for your assigned interns."
        icon={AlertTriangle}
        action={
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm ring-1 ring-gray-100">
        ...
        {!loading && rows.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            ...
            <p className="text-xs text-gray-400">Try adjusting your search or filters, or report a new issue.</p>
            {hasFilters && (
                ...
                <TableBody>
                  {rows.map((issue) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="max-w-[360px] truncate text-sm text-gray-600" title={issue.issues}>
                          {issue.issues}
                        </div>
                      </TableCell>
                      <TableCell>
