...
const REPORTS = [
  {
    ...
    desc: "KPIs for assigned interns and monitoring",
    icon: BarChart3,
    tone: "green",
...
export default function HteReportPage() {
  ...
  const handlePrint = useCallback(
    (reportKey = "overview") => {
      ...
      const printUrl = `/hte/reports/print?${params.toString()}`;
      const ensureIframe = () => {
  ...
  return (
    <HteLayout>
      <PageHeader
        icon={BarChart3}
        title="Reports"
        subtitle={`Official reports for ${data?.hte?.name || data?.institute?.name || "your establishment"} — filter by academic year`}
        action={
          <Button
