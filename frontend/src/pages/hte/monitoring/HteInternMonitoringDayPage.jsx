...
export default function HteInternMonitoringDayPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [internRes, journalsRes] = await Promise.all([
        ...
        api.get(`/hte/interns/${uuid}/monitoring?date=${dateParam}`),
      ]);
  ...
  function goToDay(ymd) {
    navigate(`/hte/monitoring/${uuid}/${ymd}`);
  }
  ...
  async function confirmVerify() {
    ...
    try {
      for (const type of targets) {
        await api.post(`/hte/interns/${uuid}/monitoring/verify`, { type, date: dateParam });
      }
      toast.success("Journal(s) verified");
      setVerifyOpen(false);
      await load();
    } catch (err) {
  ...
  async function flagRecords() {
    ...
    try {
      for (const type of targets) {
        await api.post(`/hte/interns/${uuid}/monitoring/flag`, {
          type,
          date: dateParam,
          remarks: remarks.trim(),
        });
      }
      toast.success("Journal(s) flagged");
      setRemarks("");
      setFlagOpen(false);
      await load();
    } catch (err) {
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm ring-1 ring-gray-100">
          ...
          <Button asChild variant="outline" className="mt-4 h-10 rounded-xl text-green-700">
            <Link to={`/hte/monitoring/${uuid}`}>Back to calendar</Link>
          </Button>
        </div>
      ) : intern ? (
        <>
          ...
          <section className="space-y-5">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm ring-1 ring-gray-100 sm:p-5">
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
                ...
                >
                  <ShieldCheck size={16} /> Verify journals…
                </Button>
                <Button
                  type="button"
                  variant="outline"
          ...
          <Dialog open={verifyOpen} onOpenChange={setVerifyOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>Verify journals</DialogTitle>
                <DialogDescription>
                  Select the journal(s) to mark as verified for {formatDate(dateParam)}.
                </DialogDescription>
              </DialogHeader>
          ...
          <Dialog open={flagOpen} onOpenChange={setFlagOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>Flag journals</DialogTitle>
                <DialogDescription>
                  Select the journal(s) to flag for {formatDate(dateParam)} and add your remarks.
                </DialogDescription>
              </DialogHeader>
              ...
              <Textarea
                ...
                rows={3}
                maxLength={1000}
                placeholder="Required — explain why this journal is flagged…"
