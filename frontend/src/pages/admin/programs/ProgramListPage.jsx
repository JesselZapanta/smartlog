...
export default function ProgramListPage() {
  ...
  const loadPrograms = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load programs", { description: firstErrorMessage(err) });
    } finally {
      setLoading(false);
    }
  }, [page, instituteFilter, status, debouncedSearch, order]);
  ...
  useEffect(() => {
    if (refreshTick > 0) loadPrograms();
  }, [refreshTick, loadPrograms]);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success("Program deleted", { description: `${deleteTarget.name} was removed.` });
  ...
  const hasFilters = Boolean(search) || instituteFilter !== "all" || status !== "all";

  return (
    <AdminLayout>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-green-950 sm:text-3xl">Programs</h1>
          <p className="mt-1 text-sm text-gray-500">Academic programs offered by each institute.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm ring-1 ring-gray-100">
        ...
        {!loading && programs.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            ...
            <p className="text-sm font-semibold text-gray-700">No programs found</p>
              ...
              {programs.map((program) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-xs text-gray-400">#{program.id}</span>
                    <StatusPill status={toActiveValue(program.is_active)} />
                  </div>
                  <p className="mt-2.5 truncate text-sm font-semibold text-gray-900">{program.name}</p>
              ...
              <Table>
                ...
                <TableBody>
                  {programs.map((program) => (
                    ...
                    >
                      <TableCell>
                        <span className="font-mono text-sm font-semibold text-green-700">#{program.id}</span>
                      </TableCell>
                      <TableCell>
        ...
        {!loading && meta && meta.total > 0 && (
          <div className="flex flex-col gap-3 border-t border-gray-100 bg-gray-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <p className="text-sm text-gray-500">
              ...
              <span className="font-semibold text-gray-700">{meta.total}</span> programs
            </p>
