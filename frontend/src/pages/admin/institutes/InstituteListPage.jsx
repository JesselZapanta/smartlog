...
import InstituteFormDialog from "@/pages/admin/institutes/InstituteFormDialog.jsx";
import { statusLabel, statusOptions, statusTone, toActiveValue } from "@/pages/admin/institutes/constants.js";
...
export default function InstituteListPage() {
  ...
  const loadInstitutes = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load institutes", { description: firstErrorMessage(err) });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (refreshTick > 0) loadInstitutes();
  }, [refreshTick, loadInstitutes]);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success("Institute deleted", { description: `${deleteTarget.name} was removed.` });
      setDeleteTarget(null);
      if (institutes.length === 1 && page > 1) {
        setPage(page - 1);
      } else {
        setRefreshTick((tick) => tick + 1);
      }
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-green-950 sm:text-3xl">Institutes</h1>
          <p className="mt-1 text-sm text-gray-500">Academic institutes of Tangub City Global College.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm ring-1 ring-gray-100">
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-green-50 hover:bg-green-50">
                  ...
                  <TableHead className="text-[11px] font-bold uppercase tracking-wider text-green-700">Name</TableHead>
                  <TableHead className="text-[11px] font-bold uppercase tracking-wider text-green-700">Description</TableHead>
        ...
        {!loading && institutes.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            ...
            <p className="text-sm font-semibold text-gray-700">No institutes found</p>
              ...
              {institutes.map((institute) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-xs text-gray-400">#{institute.id}</span>
                    <StatusPill status={toActiveValue(institute.is_active)} />
                  </div>
                  <p className="mt-2.5 truncate text-sm font-semibold text-gray-900">{institute.name}</p>
              ...
              <Table>
                ...
                <TableBody>
                  {institutes.map((institute) => (
                    ...
                    >
                      <TableCell>
                        <span className="font-mono text-sm font-semibold text-green-700">#{institute.id}</span>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm font-semibold text-gray-900">{institute.name}</p>
                      </TableCell>
                      <TableCell>
        ...
        {!loading && meta && meta.total > 0 && (
          <div className="flex flex-col gap-3 border-t border-gray-100 bg-gray-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <p className="text-sm text-gray-500">
              ...
              <span className="font-semibold text-gray-700">{meta.total}</span> institutes
            </p>
